...
@api.route('/grade/export', methods=['GET'])
...
def handle_export_grade():
    ...
    if file_type == 'pdf':
        ...
        pdf_message = session.post(config.pdf_url1, data=post_data1, cookies=cookies).content.decode()
        if pdf_message.find('可打印') == -1:
            custom_abort(-3, pdf_message)
        pdf_url = session.post(config.pdf_url2, data=post_data1, cookies=cookies).content.decode().replace('\\',
                                                                                                           '').replace(
            '\"',
            '')
        if pdf_url.find('成功') != -1:
            pdf_content = session.get('http://222.31.49.139' + pdf_url.split('#')[0], cookies=cookies).content
        ...
        xls_content = session.post(config.excel_url, data=config.post_data, cookies=cookies).content
